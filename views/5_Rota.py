...
# ── SIDEBAR ──────────────────────────────────────────────────────
def render_sidebar():
    view=st.session_state.view; ws=st.session_state.week_start
    gen=get_generated_weeks(); sched=get_scheduling_weeks()
    ...
    with st.sidebar:
        ...
        if view in('week','day') and ws:
            ...
            st.markdown(
                ...
                unsafe_allow_html=True)
            budget=get_schedule_budget(ws); total_h=hrs_map.get(ws,0)
            if budget:
                pct=min(total_h/budget,1.0)
                st.markdown(f"**💰 Budget:** {total_h}h / {budget}h")
                st.progress(pct)
                if pct>=0.95: st.warning("⚠️ Near budget limit")
            else:
...
# ── VIEW 1: CALENDAR ─────────────────────────────────────────────
def show_calendar():
    sw=get_scheduling_weeks(); gw=get_generated_weeks(); hm=get_week_total_hours()
    ...
    for wr in calendar.monthcalendar(yr,mo):
        ...
        for di,dn2 in enumerate(wr):
            with cols[di]:
                ...
                hr=ws in gw; sc=ws in sw; it=curr==today
                if hr:   cc,bc,bt='cell-rota','badge-rota','✓ Rota'
                elif sc: cc,bc,bt='cell-sched','badge-sched','+ Setup'
                else:    cc,bc,bt='cell-empty','badge-empty','—'
                ...
                if st.button(bl,key=f"d_{yr}_{mo}_{dn2}",use_container_width=True,type=btyp,disabled=dis):
                    if hr: nav_to('week',sel_date=curr,week_start=ws)
                    else:  nav_to('generate',sel_date=curr,week_start=ws)
    if st.session_state.view=='generate' and st.session_state.week_start:
        _gen_panel(sw)

def _gen_panel(sw):
    ws=st.session_state.week_start; we=ws+timedelta(days=6)
    st.divider()
    if ws not in sw:
        st.warning("⚠️ No shift template for this week. Add one in the Scheduling page first."); return
        ...
        if st.button("🚀 Generate Rota",type="primary",use_container_width=True):
            with st.spinner("⏳ Optimising…"):
                try:
                    ...
                    st.success("✅ Done!"); time.sleep(0.8); nav_to('calendar')
                except Exception as e: st.error(f"Error: {e}")

# ── VIEW 2: WEEK ─────────────────────────────────────────────────
def show_week_view():
    gw=get_generated_weeks(); ws=st.session_state.week_start
    we=ws+timedelta(days=6); sn=gw.get(ws); roles=get_employee_roles()
    c1,c2,c3=st.columns([1.2,6,1.5])
    with c1:
        st.write("")
        if st.button("◀ Calendar",use_container_width=True): nav_to('calendar')
    with c2:
        ...
        if st.button("🔄 Re-generate",use_container_width=True):
            with st.spinner("Regenerating…"):
                try:
                    from scheduler_h_s import solve_rota_final_v14
                    solve_rota_final_v14(EMPLOYEE_FILE,HOLIDAY_FILE,target_weeks=[ws])
                    if f"df_{sn}" in st.session_state: del st.session_state[f"df_{sn}"]
                    st.success("Done!"); time.sleep(0.8); st.rerun()
                except Exception as e: st.error(f"Error: {e}")
    st.divider()
    if not sn: st.error("No rota found."); return
    df=load_week_rota(sn)
    if df is None: st.error("Could not load rota."); return
    dc=[c for c in df.columns if c not in('Name','Employee ID','Total Weekly Hours')]
    ...
    for i,(_,row) in enumerate(df.iterrows()):
        role=roles.get(str(row['Name']),'Associate'); clr=ROLE_COLORS.get(role,DRC)
        nt[i].markdown(
            ...
            unsafe_allow_html=True)
    sk=f"df_{sn}"
    if sk not in st.session_state: st.session_state[sk]=df.copy()
    ...
    if not ed.equals(st.session_state[sk]):
        ed=recalc(ed); st.session_state[sk]=ed; st.rerun()
    st.divider()
    with st.expander("📥 Download Options"):
        dc2a,dc2b,dc2c=st.columns(3)
        with dc2a:
            buf=io.BytesIO()
            with pd.ExcelWriter(buf,engine='openpyxl') as w: ed.to_excel(w,index=False,sheet_name=sn)
        ...
        with dc2c:
            if os.path.exists(OUTPUT_FILE):
                with open(OUTPUT_FILE,'rb') as f: st.download_button("⬇️ Full Workbook",f.read(),"Final_Rota_Full.xlsx")

# ── VIEW 3: DAY ──────────────────────────────────────────────────
def show_day_view():
    gw=get_generated_weeks(); sd=st.session_state.selected_date
    ws=st.session_state.week_start; sn=gw.get(ws); roles=get_employee_roles()
    c1,c2=st.columns([1.2,7])
    with c1:
        st.write("")
        if st.button("◀ Week View",use_container_width=True): nav_to('week')
    with c2:
    ...
    if not dc: st.warning("No data for this day."); return
    is_ev='[Event' in dc
    ...
    m1,m2,m3,m4=st.columns(4)
    m1.metric("👥 Working",len(wk)); m2.metric("🔴 Off",len(off))
    m3.metric("⏱️ Hours",f"{wk['Hours'].sum():.0f}h"); m4.metric("🎫 Event","Yes" if is_ev else "No")
    if is_ev:
    ...
    if wk.empty:
        ...
        ws2=min(sts) if sts else 7; we2=max(ens) if ens else 22; win=max(we2-ws2,1)
        ...
        for _,row in wk.iterrows():
            sh2=str(row['Shift'])
            if ' - ' not in sh2: continue
            ss,es=sh2.split(' - '); sh3=int(ss.split(':')[0]); eh3=int(es.split(':')[0])
            if eh3==0: eh3=24
            hrs2=eh3-sh3
            lp=(sh3-ws2)/win*100; wp=hrs2/win*100
            role=row['Role']; clr=ROLE_COLORS.get(role,DRC)
    ...
    with cw:
        ...
        for _,r in wk.iterrows():
            role=r['Role']; clr=ROLE_COLORS.get(role,DRC)
    ...
    with co:
        ...
        if not off: st.success("All staff working!")
        else:
            for nm in off:
                role=roles.get(str(nm),'Associate'); clr=ROLE_COLORS.get(role,DRC)
    ...
    st.divider()
    pc,nc=st.columns(2)
    pd2=sd-timedelta(days=1); nd=sd+timedelta(days=1)
    with pc:
...
if   view in('calendar','generate'): show_calendar()
elif view=='week':                    show_week_view()
elif view=='day':                     show_day_view()